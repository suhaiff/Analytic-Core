...
export const Landing: React.FC<LandingProps> = ({ onFileUpload, onGoogleSheetImport, onSqlDatabaseImport, savedDashboards, onLoadDashboard, onDeleteDashboard, onLogout, user }) => {
            ...
            <div className="flex flex-col gap-4 sm:gap-6 md:gap-8">
              ...
              <div className="flex flex-col gap-3">
                ...
                <button
                  ...
                  className={`w-full py-4 px-6 rounded-2xl border ${colors.borderPrimary} ${colors.bgSecondary} hover:${colors.bgTertiary} transition-all flex items-center justify-center gap-3 group shadow-lg hover:shadow-blue-500/10 hover-lift active-press`}
                >
                  <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                    <FileText className="w-5 h-5 text-blue-500" />
                  </div>
                  <div className="text-left flex-1">
                    <div className={`text-sm font-bold ${colors.textPrimary}`}>Connect SQL Database</div>
                    <div className={`text-xs ${colors.textMuted}`}>Live import from MySQL or PostgreSQL</div>
                  </div>
                  <ChevronRight className={`w-5 h-5 ${colors.textMuted} group-hover:translate-x-1 transition-transform`} />
                </button>
              </div>
