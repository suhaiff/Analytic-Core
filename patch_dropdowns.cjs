...
// We need an onDeleteMeasure function in ChartBuilder to pass to these dropdowns.
// We can define it right below handleSaveMeasure or similar.
const delFunc = `
...
// Now replace ALL the native selects and the one manual Metric menu with ColumnSelectDropdown
// 1. manualConfig.xAxisKey
...
// 2. manualConfig.dataKey (The big one I previously made custom inline)
const dataKeyTarget = /<div className="relative" ref=\{manualMetricMenuRef\}>[\s\S]*?<\/div>\s*<\/div>\s*<!-- Aggregation -->/;
// Wait, regex might fail to match accurately on such a huge block. Let's use string replace.
// Actually, let's find the `ref={manualMetricMenuRef}` block.
const startIndex = d.indexOf('<div className="relative" ref={manualMetricMenuRef}>');
if (startIndex !== -1) {
    const endIndex = d.indexOf('{/* Aggregation */}', startIndex);
    if (endIndex !== -1) {
        d = d.slice(0, startIndex) + `<ColumnSelectDropdown value={manualConfig.dataKey || ''} onChange={(val) => setManualConfig({...manualConfig, dataKey: val})} allColumns={allColumns} customMeasures={customMeasures} onEditMeasure={(m) => { setMeasureToEdit(m); setIsMeasureModalOpen(true); }} onDeleteMeasure={handleDeleteMeasure} theme={theme} colors={colors} />\n                                    </div>\n                                </div>\n\n                                ` + d.slice(endIndex);
    }
}

// 3. manualConfig.yAxisKey
...
// 5. manualConfig.forecastDateColumn
...
console.log('Dropdowns converted');
